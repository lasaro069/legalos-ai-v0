...
import { actualizarActuacion } from '../../../../acciones'
...
export default async function EditarActuacionPage({
  params,
  searchParams,
}: {
  params: { id: string, actuacionId: string },
  ...
  const { data: actuacion, error: actError } = await supabase
    .from('actuaciones')
    .select('*')
    .eq('id', params.actuacionId)
    .eq('expediente_id', params.id)
    .single()

  if (actError || !actuacion) {
    notFound()
  }

  // Format date for <input type="date">
  let fechaStr = ''
  if (actuacion.fecha_juridica) {
    // Depending on DB format (YYYY-MM-DD)
    fechaStr = actuacion.fecha_juridica.split('T')[0]
  }
  ...
  return (
    <div className="max-w-3xl mx-auto pb-12">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">Editar Actuación</h2>
          <p className="text-sm text-slate-500 mt-1">
            Expediente: <strong>{expediente.nombre}</strong> {expediente.radicado && `(${expediente.radicado})`}
          </p>
        </div>
      ...
      <form action={actualizarActuacion} className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 flex flex-col gap-6">
        <input type="hidden" name="expediente_id" value={expediente.id} />
        <input type="hidden" name="actuacion_id" value={actuacion.id} />

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="flex flex-col gap-2 md:col-span-2">
            <label htmlFor="titulo" className="font-semibold text-sm text-slate-800">Título de la Actuación *</label>
            <input type="text" id="titulo" name="titulo" defaultValue={actuacion.titulo} required className="border border-slate-300 rounded-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-legal-blue/50" />
          </div>

          <div className="flex flex-col gap-2">
            <label htmlFor="tipo" className="font-semibold text-sm text-slate-800">Tipo de Actuación *</label>
            <select id="tipo" name="tipo" defaultValue={actuacion.tipo} required className="border border-slate-300 rounded-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-legal-blue/50 bg-white capitalize">
          ...
          <div className="flex flex-col gap-2 md:col-span-2">
            ...
            <textarea id="descripcion" name="descripcion" defaultValue={actuacion.descripcion || ''} rows={4} className="border border-slate-300 rounded-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-legal-blue/50"></textarea>
          </div>
          
          <div className="flex flex-col gap-2 md:col-span-2 bg-slate-50 p-6 rounded-xl border border-dashed border-slate-300">
            <label htmlFor="documento" className="font-bold text-sm text-slate-800 mb-1">Reemplazar Documento (Opcional)</label>
            {actuacion.documento_url && (
              <p className="text-xs text-amber-600 mb-2 font-medium">
                Actualmente hay un documento asociado. Subir uno nuevo lo reemplazará.
              </p>
            )}
            <input 
              type="file" 
